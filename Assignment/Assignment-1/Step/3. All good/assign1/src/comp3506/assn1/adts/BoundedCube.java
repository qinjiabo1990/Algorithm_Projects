...
public class BoundedCube<T> implements Cube<T> {
	...
	private Object currentDequeue = null;
	...
	public boolean remove(int x, int y, int z, T element) throws IndexOutOfBoundsException{
		...
		int currentAircraftQueueLength = innerIndex.aircraftQueue.size();
		int currentTempQueue = tempQueue.size();
		int i = 0; 
		int j = 0;
		...
		if((x < 0 || x > length) && (y < 0 || y > breadth) && (z < 0 && z > height)){
			...
			while(i < currentAircraftQueueLength){
			...
			currentTempQueue = tempQueue.size();
			while(j < currentTempQueue){
				innerIndex.aircraftQueue.enqueue(tempQueue.dequeue());
				j++;
			}
		}
		return remove;
